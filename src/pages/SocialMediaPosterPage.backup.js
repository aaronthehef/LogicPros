...
export const SocialMediaPosterPage = () => {
  const [aiMetadata, setAiMetadata] = useState({
    visualSuggestion: '',  // From instagram_visual_suggestion - shared for all platforms
    generatedAt: null,
    source: 'n8n'
  });
  ...
  const [posts, setPosts] = useState([
    ...
    },
    {
      ...
      platform: 'Facebook',
      content: '',
      linkUrl: '',          // NEW: From facebook_link_url
      imageUrls: [],
      notes: '',
      aiMetadata: {}
    },
    {
      ...
      platform: 'Twitter/X',
      content: '',
      thread: [],           // NEW: From twitter_thread
      imageUrls: [],
      notes: '',
      aiMetadata: {}
    },
    {
      ...
      platform: 'LinkedIn',
      content: '',
      visibility: 'PUBLIC',           // NEW: From linkedin_visibility
      lifecycleState: 'PUBLISHED',    // NEW: From linkedin_lifecycle_state
      distribution: {                 // NEW: From linkedin_distribution_json
  ...
  const [blogPost, setBlogPost] = useState({
    title: '',
    content: '',
    excerpt: '',          // NEW: From blog_excerpt
    slug: '',             // NEW: From blog_slug
    status: 'draft',      // NEW: From blog_status
    featuredImageUrl: '',
    aiMetadata: {}
  });
  ...
  const handlePostFieldChange = useCallback((postId, field, value) => {
    setPosts(prevPosts =>
      prevPosts.map(post =>
          ...
          (snapshot) => {
            ...
            if (addedDocs.length > 0) {
              ...
              if (postCreatedAt > workflowStartTime) {
                ...
                if (generatedData) {
                  // Load shared AI metadata (visual suggestion)
                  setAiMetadata({
                    visualSuggestion: generatedData.instagram_visual_suggestion || '',
                    ...
                    switch (post.platform) {
                      ...
                      case 'LinkedIn':
                        ...
                        if (generatedData.socialMedia?.linkedin?.post || generatedData.linkedin_post) {
                          ...
                          if (generatedData.linkedin_distribution_json) {
                            try {
                              distribution = JSON.parse(generatedData.linkedin_distribution_json);
                            } catch (e) {
                              console.error('Failed to parse linkedin_distribution_json:', e);
                            }
                          }
